        ...
        <ul class="navbar-nav">
                    ...
                    @foreach ($applies as $apply)
                        <a class="dropdown-item preview-item">
                            <div class="preview-item-content">
                                <p class="mb-0" style="font-weight: 500"> {{ $apply->name }} {{ $apply->lastname }} <span class="text-small text-muted">apply on</span> {{ $apply->course->title_en }}
            ...
            <li class="nav-item dropdown d-none d-sm-flex">
                <a class="nav-link count-indicator dropdown-toggle" id="messageDropdown" href="#"
                    ...
                    @if ($messages->count() != 0)<span class="count count-varient2">
                            {{ $messages->count() }} </span>@endif
                </a>
                <div class="dropdown-menu navbar-dropdown navbar-dropdown-large preview-list"
                            ...
                            <a href="message/detail/{{ $message->id }}" class="dropdown-item preview-item">
                                <div class="preview-item-content flex-grow">
                                    ...
                                    <p class="text-small text-muted ellipsis mb-0">
                                        {{ Str::substr($message->text, 0, 25) }}</p>
                                </div>
